...
import { getCompanyAddresses } from "../../functions/LocationManagement/LocationManagement";
...
//   const [shortDescription, setShortDescription] = useState("");
  ...
  const [modal_edit, setmodal_edit] = useState(false);
    const handleTog_edit = (_id) => {
      ...
      getSpecialityManagement(_id)
        .then((res) => {
          ...
          console.log(JSON.parse(res.Location).length);
          // if(JSON.parse(res.Location).length>0){
    ...
    const Editvalidate=(SpecialityName)=>{
      const errors={};
      if (!/^[A-Za-z\s]+$/.test(SpecialityName)) {
        errors.SpecialityName = "SpecialityName should contain only alphabets";
        setEditSN(true);
      }  
      if (SpecialityName === "") {
        
        errors.SpecialityName = "SpecialityName is required";
        setEditSN(true);
      }
...
return errors;
  ...
  const handleClick = (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      try{
        ...
        formdata.append("Location",JSON.stringify(selectedLocationsArray));
        formdata.append("DoctorSpeciality",docspeciality);
        formdata.append("BlockQuote",JSON.stringify(blockquote));
  ...
  const handleCheckboxChange = (address) => {
    ...
    if (newCheckedItems.includes(address)) {
      newCheckedItems.splice(newCheckedItems.indexOf(address), 1); // uncheck if already checked
    } else {
      newCheckedItems.push(address); // check if not checked
      const index = selectedLocationsArray.findIndex(item => item.address === address);
    if (index === -1) {
      // Address does not exist in the array, so append it
      setSelectedLocationsArray([...selectedLocationsArray, { address }]);
    } else {
      // Address already exists, so remove it
      const updatedArray = [...selectedLocationsArray];
      updatedArray.splice(index, 1);
      setSelectedLocationsArray(updatedArray);
    }
    console.log("selelele",selectedLocationsArray);
    }
    setCheckedItems(newCheckedItems);
    console.log("refeaefcz",selectedLocationsArray);
  };
  // const [selectedLocationsArray,setSelectedLocationsArray]=useState([]);
  // const handleCheckboxChange = (address) => {
  //   const index = selectedLocationsArray.findIndex(item => item.address === address);
 ...
 const handleDelete1=async(index,e)=>{
  ...
  setDetails1(updatedDetails1);
  setSelectedLocationsArray(updatedDetails1);
  }
...
console.log(imageurl);
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb maintitle="Speciality Management" title="Speciality Management" pageTitle="Speciality Management" />
                ...
                <CardHeader>
                  <Row className="g-4 mb-1">
                    <Col className="col-sm" lg={4} md={6} sm={6}>
                      <h2 className="card-title mb-0 fs-4 mt-2">Speciality Management</h2>
                    </Col>
                    <Col lg={4} md={6} sm={6}>
                      <div
                        style={{
                                  ...
                                  <Col lg={6}>
                                    <div className="form-floating mb-3">
                                      <Input
                                        // key={"productName_" + _id}
                                        type="text"
                                        // className={validDoctorSpeciality}
                                        placeholder="Enter blog title"
                                        required
                                        name="solarrooftype"
                                        value={docspeciality}
                                  ...
                                  <Col lg={6}>
                                    <div className="form-floating mb-3">
                                      ...
                                      <Label>
                                        ...
                                        {/* <span className="text-danger">*</span> */}
                                      {/* </Label> */}
                                      {/* <button className="btn btn-primary mt-2" onClick={isUpdate ? handleLocationUpdate : handleAddLocation}>
      ...
      <React.Fragment key={index}>
        <tr>
          ...
          <input
                                  type="checkbox"
                                  size={"15px"}
                                  checked={checkedItems.includes(item.address)}
                                  onChange={() => handleCheckboxChange(item.address)}
                                />
            {/* <button className="btn btn-success me-2" onClick={(e) => handleUpdate1(index,e)}>Update</button>*/}
            {/* <button className="btn btn-danger ms-2"onClick={(e) => handleDelete1(index,e)}>Delete Location</button>  */}
          </td>
        </tr>
                                  ...
                                  <Col lg={12}>
                                    ...
                                    <div className="form-floating mb-3">
                                      <Input
                                        ...
                                        required
                                        name="specblockquote"
                                        value={specblockquote}
                                        onChange={(e) => {
                                          setSpecBlockQuote(e.target.value);
                                        }}
                                      />
                                      <Label>
                                        Quote{" "}
                                      ...
                                      <button className="btn btn-primary mt-2" onClick={isUpdate1 ? blockQuoteUpdate : handleAddBlockQuote}>
  {isUpdate1 ? "Update Quote" : "Add Quote"}
</button>
                                      <Table>
  <thead>
    <tr>
  ...
  <tbody>
    {blockquote.map((item, index) => (
      <React.Fragment key={index}>
        <tr>
          <td>{item.blockquote}</td>
          <td className="col-lg-6">
            <button className="btn btn-success me-2" onClick={(e) => handleUpdate2(index,e)}>Update</button>
            <button className="btn btn-danger ms-2"onClick={(e) => handleDelete2(index,e)}>Delete</button>
          </td>
        </tr>
