...
const handleDeleteClick2 = (index) => {
  ...
  const handleLocation = (e) => {
    ...
    let erros = locationvalidate(when,where);
    console.log("location errorssssss",erros);
    setFormErrors(erros);
    setIsSubmit1(true);
 ...
 const locationvalidate=(when,where)=>{
  ...
  const validClassName =
    ...
    const validClassWhere =
    errWhere && isSubmit ? "form-control is-invalid" : "form-control";
    const validClassWhen =
    errWhen && isSubmit ? "form-control is-invalid" : "form-control";
  ...
  const getdocspecialitybyid=async(id)=>{
    const get= await axios
    .get(
      `${process.env.REACT_APP_API_URL_SHELDON}/api/auth/getDoctorSpecialityById/${id}`).then((response)=>{
        let res=response[0];
        console.log("res 787",response);
        setSpecialityNameOther(response.DoctorSpeciality);
      });
  }
  const handleSpecialityChange = (event) => {
    ...
    getdocspecialitybyid(event.value);
