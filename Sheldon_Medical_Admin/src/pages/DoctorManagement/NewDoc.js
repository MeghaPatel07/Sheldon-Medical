...
const NewDoc = () => {
  ...
  const fetchUsers = async () => {
    ...
    //       setDoctors(res.data);
    //       setBlogs(res.data)
    //       // let combinedData = res.data.map((doctor, index) => ({
    //       //   ...doctor,
    //       //   specialityName: res.specialities[index].SpecialityName
    //       // }));
  
    //       console.log("res.data.S", res.data);
    //       console.log("doctorsss specialitiessss", doctors);
    ...
    let resp=await axios
      ...
      console.log("response in getdoctorsss",resp.data);
      // let res = resp[0];
            // console.log(res)
            setLoading(false);
            // setDoctors(resp); 
  ...
  const handleTog_edit = (_id) => {
    ...
    getDoctorById(_id)
      ...
      // Output the idsArray
        setSpeciality(res.SpecialityName.id);
...
const handleDeleteClick2 = (index) => {
  ...
  const handleClick = (e) => {
    ...
    // const { SpecialityName, docname, detail, where, when, docimg } = values;
    let errors = validate(SpecialityName, docname);
    ...
    createDoctor(values.SpecialityName, values.docname,values.detail,finalloc, values.photo, values.specialityNameOther)
      .then((res) => {
        console.log(res);
        // setmodal_list(!modal_list);
  ...
  const handleUpdate = (e) => {
    ...
    if (Object.keys(erros).length === 0) {
      ...
      updateDoctorManagement(_id, values.SpecialityName, docname, values.detail, editlocid, values.photo, specialityNameOther)
        .then((res) => {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb maintitle="Add New Speciality" title="Service" pageTitle="Blogs" />
                ...
                <CardHeader>
                  <Row className="g-4 mb-1">
                    <Col className="col-sm" lg={4} md={6} sm={6}>
                      <h2 className="card-title mb-0 fs-4 mt-2">Add New Service</h2>
                    </Col>
                    <Col lg={4} md={6} sm={6}>
                      {/* <div
                        style={{
              ...
              <td>
                ...
                <button onClick={() => handleDeleteClick(index)}>Delete</button>
                                  ...
                                  <div>
                                    ...
                                    <label htmlFor="when">When:</label>
                                    <input type="text" id="when" name="when" value={values.when} onChange={handleChange} />
                                    <button onClick={handleLocationAddClick}>
                                      {editIndex !== null ? 'Update Location' : 'Add Location'}
                                    </button>
                                    ...
                                    {photoAdd ? (
                                      <img
                                        ...
                                        src={
                                          checkImagePhoto
                                            ? photoAdd
                                            : `${process.env.REACT_APP_API_URL_SHELDON}/${photoAdd}`
                                        }
                                        width="180"
                                        height="200"
                                      />
  ...
  <tr key={index}>
    ...
    <td>
      ...
      >
        ...
        >
          <span style={{ marginRight: "210px" }}>Remove Doctor</span>
        </ModalHeader>
