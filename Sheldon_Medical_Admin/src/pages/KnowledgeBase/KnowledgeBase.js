...
import {
  getNewKnowlegeBase,uploadproductImage,getKnowlegeBase,updateKnowlegeBase,listKnowlegeBase,removeKnowlegeBase,createKnowlegeBase
} from "../../functions/KnowledgeBase/KnowledgeBase"
const KnowledgeBase = () => {
...
//   const [shortDescription, setShortDescription] = useState("");
  ...
  const [errEditTT, setEditTT]=useState(false);

  const Editvalidate=(Title)=>{
    const errors={};
    if (!/^[A-Za-z\s]+$/.test(Title)) {
      errors.Title = "Title should contain only alphabets";
      setEditTT(true);
    }  
    if (Title === "") {
...
// console.log("error in " , errors.Title);
else{ setErrTT(false);}
return errors;
    ...
    const handleTog_edit = (_id) => {
      ...
      getKnowlegeBase(_id)
        .then((res) => {
          ...
          setproductImage(res.UploadFile);
          setTitle(res.Title);
  ...
  const handleClick = (e) => {
   ...
   let errors = Editvalidate(Title)
      ...
      updateKnowlegeBase(_id, formdata)
        .then((res) => {
          // setmodal_edit(!modal_edit);
          setPhotoAdd("");
          setPhotoAdd1("");
...
setTitle("");
  ...
  const validTitle =
    ...
    const validEditTitle =
    errEditTT && isSubmit ? "form-control is-invalid" : "form-control";
  ...
  const fetchCategories = async () => {
   ...
   const response= await axios
      .post(
        `${process.env.REACT_APP_API_URL_SHELDON}/api/auth/list-by-params/knowledgebase`,
        {
          skip: skip,
          per_page: perPage,
          sorton: column,
...
setTitle("");
  ...
  const col = [
    ...
    // {
    //   name: "Detail",
    //   cell: (row) => renderImage(row.Detail),
    //   sortable: true,
    ...
    // {
    //   name: "Icon",
    //   cell: (row) => renderImage(row.UploadFile),
    //   sortable: true,
    //   sortField: "serialNumber",
    //   minWidth: "150px",
    // },
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb maintitle="Knowledge Base" title="Knowledge Base" pageTitle="Knowledge Base" />
                ...
                <CardHeader>
                  <Row className="g-4 mb-1">
                    <Col className="col-sm" lg={4} md={6} sm={6}>
                      <h2 className="card-title mb-0 fs-4 mt-2">Knowledge Base</h2>
                    </Col>
                    <Col lg={4} md={6} sm={6}>
                      <div
                        style={{
     ...
     <Modal
      ...
      >
        ...
        >
          <span style={{ marginRight: "210px" }}>Remove Knowledge Base</span>
        </ModalHeader>
